...
from nodes.base import Node, llm_mini
...
ユーザーに送りたいファイルがある場合は、`description`にはファイルのありかや、カレントディレクトリに保存した旨を記載しないでください、これはユーザーに伝わる際にダウンロードリンクに変換されるためです。
...
def create_final_answerer_node(members: list[str], name: str = "FinalAnswerer") -> Node:
    ...
    chain = (
        prompt
        | llm_mini.bind_tools(tools=[function_def])
